...
public class Tokenizer<SourceT> {
  private final HybridEditorSpec<SourceT> mySpec;

  public Tokenizer(HybridEditorSpec<SourceT> spec) {
    mySpec = spec;
  }
  ...
  private class TextMatcher {
    ...
    private final CompletionItems completionItems = new CompletionItems(mySpec.getTokenCompletion(new Function<Token, Runnable>() {
      @Nullable
      @Override
    ...
    private TextMatchResult match(String text) {
      List<CompletionItem> matches = completionItems.matches(text);
      if (matches.size() == 1) {
        matches.iterator().next().complete(text).run();
        return singleMatched(tokenHolder.get());
      } else {
        boolean noWayToComplete = matches.isEmpty() && completionItems.prefixedBy(text).isEmpty();
        if (noWayToComplete) {
          return error(text);
        } else {
          return pending(text);
