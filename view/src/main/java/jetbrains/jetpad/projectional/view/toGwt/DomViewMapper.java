...
class DomViewMapper extends BaseViewMapper<DomView, Element> {
  ...
  protected void registerSynchronizers(SynchronizersConfiguration conf) {
    ...
    ReadableProperty<Vector> bounds = Properties.select(getSource().element, DomUtil::dimension, Vector.ZERO);

    conf.add(Synchronizers.forEventSource(bounds, () -> {
      getSource().validate();
    }));

    conf.add(Synchronizers.forPropsOneWay(getSource().element, value -> {
    ...
    conf.add(Synchronizers.forPropsOneWay(getSource().focused(), value -> {
