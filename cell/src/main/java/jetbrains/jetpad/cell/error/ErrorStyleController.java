...
class ErrorStyleController extends CellTrait {
  private ErrorStyler myDefaultStyler;
  private List<Function<Cell, ErrorStyler>> myStylerSuppliers;
  private Map<Cell, StyleRegistrations> myRegistrations = null;

  ErrorStyleController(ErrorStyler defaultStyler, List<Function<Cell, ErrorStyler>> customStylers) {
    myDefaultStyler = defaultStyler == null ? new ErrorStyler() : defaultStyler;
    myStylerSuppliers = (customStylers == null || customStylers.size() == 0) ? null : new ArrayList<>(customStylers);
  }

  private ErrorStyler stylerFor(Cell cell) {
    if (myStylerSuppliers != null) {
      for (Function<Cell, ErrorStyler> supplier : myStylerSuppliers) {
        ErrorStyler styler = supplier.apply(cell);
  ...
  private static class StyleRegistrations implements Disposable {
    Registration myBroken = null;
    Registration myError = null;
    Registration myWarning = null;
