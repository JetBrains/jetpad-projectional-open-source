...
public class TextEditing {
  ...
  public static Function<Cell, ErrorStyler> errorStyler() {
    final ErrorStyler textStyler = new ErrorStyler() {
      @Override
      protected Registration doApplyBroken(Cell cell) {
        return cell.set(TextCell.TEXT_COLOR, Color.RED);
      }
    };
    return new Function<Cell, ErrorStyler>() {
      @Override
      public ErrorStyler apply(Cell cell) {
